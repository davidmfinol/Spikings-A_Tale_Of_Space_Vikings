...
function OnControllerColliderHit (hit : ControllerColliderHit) {
	...
	if (hit.gameObject.tag == "small platform" && inputJump) {
	  var new_position = hit.gameObject.transform.position;
	  var old_position = tr.position;
	  tr.Translate(new_position.x - old_position.x, new_position.y - old_position.y + 1, new_position.z - old_position.z - .1);
	} else if (hammer && inputFire && (hit.gameObject.tag == "small rock" || hit.gameObject.tag == "small enemy")) {
	  hit.gameObject.SetActive(false);
	} else if (hit.gameObject.tag == "item") {
	  hit.gameObject.SetActive(false);
	  hammer = true;
	} else if (hit.gameObject.tag == "spaceship") {
	  Application.Quit();
	}
}
